...
def build_rho_vector(
    nx, ny, dx=1.0, dy=1.0, thetarange=None, intercept=False, oversampling=1.0
):
    ...
    if thetarange is None or (
        thetarange[0] == -0.5 * np.pi and thetarange[1] == 0.5 * np.pi
    ):
        thetarange = None
        ...
        _, val, _, _ = sp.optimize.fminbound(
            f, thetarange[0], thetarange[1], full_output=True
        )
        val = -val
    ...
    if intercept:
        ...
        if thetarange is None:  # default range of [-0.5pi, 0.5pi]
            ...
            f = lambda theta, scale, x, y: scale * (
                x * np.cos(theta) + y * np.sin(theta)
            )

            for x in [xmin, -xmin]:
                for y in [ymin, -ymin]:
                    _, tmp, _, _ = sp.optimize.fminbound(
                        f, thetarange[0], thetarange[1], (-1, x, y), full_output=True
                    )
...
def build_theta_vector(nx, ny, dx=1.0, dy=1.0, thetarange=None, oversampling=1.0):
...
class RadonTransform(radonc.Radon):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.oversampling = kwargs.pop("oversampling", 1.0)
    ...
    def padded_transform(self, data, theta, rho):
        ...
        if self.constraint == Constraint.No:
            raise Exception(
                "Padded radon transform only supported with X/Y constraint."
            )
        ...
        if self.pad == PadMethod.Median:
            ...
            for k in range(data.shape[1 - ax]):
                ...
                if method == IntegralMethod.Sum:
                    r[idx] += np.random.choice(
                        np.take(data, k, axis=1 - ax), size=(np.sum(idx))
                    )
    ...
    def fit_line(
        self, data, x=None, y=None, theta=None, rho=None, form="slope-intercept"
    ):
        ...
        if theta is None:
            theta, _ = build_theta_vector(
                len(x), len(y), oversampling=self.oversampling
            )
        ...
        if form == "slope-intercept":
            line_parameters = normal_to_slope_intercept(
                theta_, rho_, origin=(0.5 * (x[0] + x[-1]), 0.5 * (y[0] + y[-1]))
            )
        else:
...
def local_integrate(m, n=3, axis=0):
    ...
    """
    return fklab.signals.smooth.smooth1d(
        m, axis=axis, kernel="uniform", bandwidth=n / 2.0, normalize=False
    )
...
def test_line_fit(
    ...
    dx, dy = 10.0, 0.1
    xstart, ystart = 7.5, 25.0

    # (x0, y0, x1, y1)
    lines = [
        (0, size[1] - 1, int(0.8 * (size[0] - 1)), int(0.2 * (size[1] - 1))),
    ...
    for L in range(len(lines)):
        ...
        ax[0, L].imshow(
            ...
            interpolation="none",
            origin="lower",
            extent=[r[2][0], r[2][-1], r[1][0], r[1][-1]],
            vmin=0,
            vmax=50,
        )

        ax[0, L].plot(
            r[4], r[3], "ko", markerfacecolor="None", markersize=15, markeredgewidth=1
        )
