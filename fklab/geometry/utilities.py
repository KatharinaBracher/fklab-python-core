...
@jit(
    locals={"npoints": int_, "nvertex": int_, "i": int_, "j": int_},
    nopython=True,
    nogil=True,
)
def inpoly(nodes, xy):
    ...
    for i in range(nvertex - 1):
        for j in range(npoints):
            if (
                nodes[i, 1] <= xy[j, 1] and nodes[i + 1, 1] > xy[j, 1]
            ):  # upward crossing
                if (
                    (nodes[i + 1, 0] - nodes[i, 0]) * (xy[j, 1] - nodes[i, 1])
                    - (xy[j, 0] - nodes[i, 0]) * (nodes[i + 1, 1] - nodes[i, 1])
                ) > 0:
                    b[j] += 1
            elif (
                nodes[i, 1] > xy[j, 1] and nodes[i + 1, 1] <= xy[j, 1]
...
def point2line(line, points, clip=("normal", "normal")):
    ...
    dpoint = points - P
    ss = np.sign(
        np.sin(np.arctan2(dpoint[:, 1], dpoint[:, 0]) - np.arctan2(v[0, 1], v[0, 0]))
    )
...
def floyd_warshall(m):
    """Find all pair shortest path algorithm for weighted directed graphs.
...
def _shortest_path_recurse(nextnode, i, j):
    ...
    if intermediate >= 0:
        p = (
            _shortest_path_recurse(nextnode, i, intermediate)
            + [intermediate]
            + _shortest_path_recurse(nextnode, intermediate, j)
        )
