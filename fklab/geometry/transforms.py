...
def _scale_matrix(sxy):
    return np.array([[sxy[0], 0, 0], [0, sxy[-1], 0], [0, 0, 1]])
...
def _translation_matrix(txy):
    return np.array([[1, 0, txy[0]], [0, 1, txy[-1]], [0, 0, 1]])


def _identity_matrix():
    return np.array(np.diag(np.ones(3)))
...
class Scale(Transform):
    ...
    def _get_str(self, indent=0):
        s = self._get_indent(
            indent
        ) + "Scale by %(xfactor)f, %(yfactor)f with origin (%(xorigin)f,%(yorigin)f)" % {
            "xfactor": self._factor[0],
            "yfactor": self._factor[-1],
            "xorigin": self._origin[0],
            "yorigin": self._origin[1],
        }
        return s

    def _get_matrix(self):
        return (
            _translation_matrix(self._origin)
            @ _scale_matrix(self._factor)
            @ _translation_matrix(-self._origin)
        )
    ...
    def _get_invmatrix(self):
        return (
            _translation_matrix(self._origin)
            @ _scale_matrix(1.0 / self._factor)
            @ _translation_matrix(-self._origin)
        )

    @property
    def factor(self):
        """Scalar or [scale_x, scale_y]."""
        return self._factor
...
class Rotate(Transform):
    ...
    def _get_str(self, indent=0):
        s = self._get_indent(
            indent
        ) + "Rotate by %(angle)f radians around origin (%(xorigin)f,%(yorigin)f)" % {
            "angle": self._angle,
            "xorigin": self._origin[0],
            "yorigin": self._origin[1],
        }
...
class Translate(Transform):
    ...
    def _get_str(self, indent=0):
        s = self._get_indent(indent) + "Translate %(xoffset)f, %(yoffset)f" % {
...
class TransformStack(Transform):
    ...
    def __setitem__(self, key, value):
        if isinstance(value, Transform):
            ...
            [isinstance(x, Transform) for x in value]
        ):
            pass
