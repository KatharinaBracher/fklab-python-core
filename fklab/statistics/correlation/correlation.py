"""
=============================================================
Correlation (:mod:`fklab.statistics.correlation.correlation`)
=============================================================

.. currentmodule:: fklab.statistics.correlation.correlation
...
def xcorrn(x, y=None, lags=None, scale="none", remove_mean=False):
    ...
    if lags is None:
        lags = np.array(
            [(-yshape + 1, xshape - 1) for xshape, yshape in zip(x.shape, y.shape)],
            dtype=np.int,
        )
    else:
    ...
    if scale == "none":
        ...
        if hasNaN:
            ...
            tmp = reduce(
                np.multiply,
                [
                    np.minimum(np.minimum(sx, sy), np.minimum(sx - l, sy + l))
                    for sx, sy, l in zip(x.shape, y.shape, lags)
                ],
            )
            tmp[tmp <= 0] = 1
            c = c / tmp
