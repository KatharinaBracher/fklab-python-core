...
def convert_to_pxyabw(filename):
    ...
    try:
        ...
        if not fid.header["InputInverted"]:
            invert = -1.0  # we want to invert, so we can use max to find spike peaks
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Extract spike waveform features from Neuralynx ntt file and save as pxyabw file for use with xclust."
    )
