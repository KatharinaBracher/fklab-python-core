...
MwlHeaderValidator.set_handler(
    IntHandler,
    [
        "Argc",
        "Probe",
...
_mwl_type_map_inverse = dict((v, k) for k, v in _mwl_type_map.items())
...
def dtype2str(dt, name=None):
    ...
    s = "{name},{code},{size},{shape}".format(
        name=name, code=type_code, size=type_size, shape=shape
    )
    return s
...
def dtype2mwl(dt):
    ...
    if dt.fields is None:
        ...
        fields = list(dt.fields.items())  # get a list of all fields
...
_LOWFILTERMAP_INV = dict((v, k) for k, v in _LOWFILTERMAP.items())
_HIGHFILTERMAP_INV = dict((v, k) for k, v in _HIGHFILTERMAP.items())
...
def parseheader(h):
    ...
    next_line_length = list(map(len, h[1:]))
...
class MwlFileBase(object):
    ...
    def __init__(self, filename):
        ...
        if (
            self._expected_file_type is not None
            and self._expected_file_type != self._file_type
        ):
    ...
    def create(cls, filename, **kwargs):
        ...
        if (
            ...
            raise IOError(
                "File already exists. Use overwrite=True to overwrite existing file"
            )
...
class MwlFileFixedRecord(MwlFileBase):
    ...
    def _setup_data_access(self, mmap=False):
        ...
        if (
            self._expected_record_dtype is not None
            and self._expected_record_dtype != self._record_dtype
        ):
            raise ValueError
        ...
        self.data = BinaryFileReader(
            self.fullpath,
            self._record_dtype,
            offset=self._headersize,
            alias=self._field_aliases,
    ...
    def append_data(self, data):
        if (
            self._expected_record_dtype and self._expected_record_dtype != data.dtype
        ) or self._record_dtype != data.dtype:
...
class MwlFileDiode(MwlFileFixedRecord):
    ...
    def __init__(self, filename, field_aliases={}, field_groups={}, **kwargs):
        ...
        field_aliases = dict(
            [("time", ("timestamp", MwlTimestamp2Seconds))], **field_aliases
        )
        field_groups = dict(
            [("front", ["xfront", "yfront"]), ("back", ["xback", "yback"])],
            **field_groups
        )

        super(MwlFileDiode, self).__init__(
            filename, field_aliases=field_aliases, field_groups=field_groups, **kwargs
        )
...
class MwlFileEeg(MwlFileFixedRecord):
    ...
    def __init__(self, filename, field_aliases={}, field_groups={}, **kwargs):
        ...
        field_aliases = dict(
            [
                ("time", ("timestamp", self.convert_timestamp)),
                ("data", ("data", self.convert_signal)),
            ],
            **field_aliases
        )
        # field_groups = dict(**field_groups)

        super(MwlFileEeg, self).__init__(
            filename,
            field_aliases=field_aliases,
            field_groups=field_groups,
            mmap=kwargs.pop("mmap", False),
        )
    ...
    def _prepare_data_access(self):
        ...
        self._gains = np.array(
            [
                self._header["channel " + str(k) + " ampgain"]
                for k in range(self._nchannels)
            ]
        )
        self._sampling_rate = self._header["rate"] / self._nchannels

        self._expected_record_dtype = np.dtype(
            [
                ("timestamp", np.uint32),
                ("data", np.int16, self._nchannels * self._nsamples),
            ]
        )
        self._cast_record_dtype = np.dtype(
            [
                ("timestamp", np.uint32),
                ("data", np.int16, (self._nsamples, self._nchannels)),
            ]
        )

    def convert_timestamp(self, timestamp):

        return self.expand_timestamp(
            timestamp=MwlTimestamp2Seconds(timestamp), scale=1.0 / self._sampling_rate
        )
    ...
    def convert_signal(self, data):
        ...
        if self._scale_signal:
            scale = (
                scale
                * ad_bit_volts(self._gains)
                * _units_scale_factor[self._signal_units]
            )
    ...
    def _prepare_header(cls, **kwargs):
        ...
        kwargs["dtype"] = np.dtype(
            [("timestamp", np.uint32), ("data", np.int16, nchannels * nsamples)]
        )
    ...
    def units(self, value):
        if not value in list(_units_scale_factor.keys()):
            raise ValueError
...
class MwlFileEvent(MwlFileFixedRecord):
    ...
    def __init__(self, filename, field_aliases={}, field_groups={}, **kwargs):
        ...
        super(MwlFileEvent, self).__init__(
            filename, field_aliases=field_aliases, field_groups=field_groups, **kwargs
        )
    ...
    def _prepare_data_access(self):
        ...
        self._expected_record_dtype = np.dtype(
            [("timestamp", np.uint32), ("string", np.uint8, self._string_size)]
        )
        self._cast_record_dtype = np.dtype(
            [("timestamp", np.uint32), ("string", np.str_, self._string_size)]
        )
    ...
    def _prepare_header(cls, **kwargs):
        ...
        kwargs["dtype"] = np.dtype(
            [("timestamp", np.uint32), ("string", np.uint8, string_size)]
        )
...
class MwlFileWaveform(MwlFileFixedRecord):
    ...
    def __init__(self, filename, field_aliases={}, field_groups={}, **kwargs):
        ...
        field_aliases = dict(
            [
                ("time", ("timestamp", MwlTimestamp2Seconds)),
                ("waveform", ("waveform", self.convert_waveform)),
            ],
            **field_aliases
        )
        field_groups = dict(**field_groups)

        super(MwlFileWaveform, self).__init__(
            filename,
            field_aliases=field_aliases,
            field_groups=field_groups,
            mmap=kwargs.pop("mmap", False),
    ...
    def _prepare_data_access(self):
        ...
        self._gains = np.array(
            [
                self._header[
                    "channel " + str(k + self._probe * self._nchannels) + " ampgain"
                ]
                for k in range(self._nchannels)
            ]
        )
        ...
        self._expected_record_dtype = np.dtype(
            [
                ("timestamp", np.uint32),
                ("waveform", np.int16, self._nchannels * self._nsamples),
            ]
        )
        self._cast_record_dtype = np.dtype(
            [
                ("timestamp", np.uint32),
                ("waveform", np.int16, (self._nsamples, self._nchannels)),
            ]
        )
    ...
    def convert_waveform(self, waveform):
        ...
        if self._scale_waveform:
            scale = (
                scale
                * ad_bit_volts(self._gains).reshape(1, self._nchannels, 1)
                * _units_scale_factor[self._signal_units]
            )
    ...
    def _prepare_header(cls, **kwargs):
        ...
        nsamples = int(kwargs.get("nsamples", 32)) * nchannels
        kwargs["dtype"] = np.dtype(
            [("timestamp", np.uint32), ("waveform", np.int16, nchannels * nsamples)]
        )
