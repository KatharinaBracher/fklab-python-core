"""
===================================================
Binary file reading (:mod:`fklab.io.common.binary`)
===================================================
...
class accessor:
    def __init__(
        ...
        if self._record_processor is None and len(self._fields) > 1:
            raise ValueError(
                "Only one source field allowed if no record level processing function is specified"
            )
        ...
        if record_shape is None:
            ...
            if not isinstance(self._record_shape, tuple) and not all(
                [isinstance(x, int) for x in self._record_shape]
            ):
                raise ValueError("Invalid record shape")

        if sample_dim is not None and (
            not isinstance(sample_dim, int)
            or sample_dim < 0
            or sample_dim >= len(self._record_shape)
        ):
    ...
    def __init__(self, filename, dtype, offset=0, accessors={}):
        ...
        if not sys.maxsize > 2 ** 32:
            raise NotImplementedError(
                "New style Neuralynx IO is only supported for 64 bit systems. Use the legacy module if you are running 32 bit system."
            )
        ...
        if nrecords == 0:
            self._memorymap = np.memmap(
                filename, mode="r", dtype=dtype, offset=0, shape=(nrecords,)
            )
        else:
            self._memorymap = np.memmap(
                filename, mode="r", dtype=dtype, offset=offset, shape=(nrecords,)
            )
...
class _DataProxy_MemoryMap(object):
    def __init__(self, memorymap, accessors=None, indexer=None):
        ...
        if self._indexer is not None and self._accessors is not None:
            if not all(
                [
                    c.nsamples() == self._indexer.nsamples_per_record()
        ...
        if self._accessors is not None:
            if self._indexer is None:
                self.shape = [
                    (self._nrecords,) + c.record_shape() for c in self._accessors
                ]
            else:
                self.shape = [
                    (self._indexer.nsamples(),)
                    + tuple(
                        [
                            x
                            for (idx, x) in enumerate(c.record_shape())
                            if idx != c.sample_dim()
                        ]
                    )
                    for c in self._accessors
        ...
        if not self._accessors is None and len(self._accessors) > 1:
            self._return_factory = namedtuple(
                "data", [c.label() for c in self._accessors]
            )
    ...
    def __getitem__(self, key):
        ...
        if self._indexer is None:  # load by record
            ...
            if isinstance(
                key, int
            ):  # make sure that loading data below keeps the first records axis
                key = slice(key, key + 1)

            in_data = {
                fld: np.array(self._memorymap[fld][key]) for fld in fields_to_read
            }
            ...
            if False:  # all( [ c['record_func'] is None for c in self._accessors ] ):
                ...
                unique_record_indices, unique_inverse = np.unique(
                    record_indices, return_inverse=True
                )

                # load records
                in_data = {
                    fld: np.array(self._memorymap[fld][unique_record_indices])
