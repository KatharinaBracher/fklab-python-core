...
class HeaderValidator:
    def __init__(self, default=None):
        ...
        if (
            not default is None
            and not isinstance(default, ValueHandler)
            and not issubclass(default, ValueHandler)
        ):
            raise ValueError

    def set_handler(self, handler, targets):
        if not isinstance(handler, ValueHandler) and not issubclass(
    ...
    def _get_handler(self, target):
        if target in list(self._handlers.keys()):
...
class subheader(object):
    ...
    def push(self, key, value):
        if key in list(self._parameters.keys()):
            raise KeyError
        self[key] = value
    ...
    def get_comments(self):
        comments = [
            v for k, v in self._parameters.items() if self._comment_pattern.match(k)
        ]
        return comments
...
class header(object):
    ...
    def _create_subheader(self, parameters=None):
        if parameters is None:
            ...
            h = subheader(
                parameters=iter(parameters.items()), validator=self._validator
            )
        else:
    ...
    def serialize(
        ...
        for k in self._subheaders:
            s += k.serialize(
                parameter_format=parameter_format, comment_format=comment_format
            )
    ...
    def items(self):
        return [list(k.items()) for k in self._subheaders]

    def parameters(self):
        return [k.parameters() for k in self._subheaders]

    def values(self):
        return [list(k.values()) for k in self._subheaders]

    def iteritems(self):
        return itertools.chain(*[list(k.items()) for k in self._subheaders])

    def iterparameters(self):
        return itertools.chain(*[k.parameters() for k in self._subheaders])

    def itervalues(self):
        return itertools.chain(*[list(k.values()) for k in self._subheaders])
