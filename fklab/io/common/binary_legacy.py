...
class _DataProxy_Direct(object):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            ...
            nrows = len(indices)
            if np.any(
                np.logical_or(indices < 0, indices >= self._fileinfo["nrecords"])
            ):
        ...
        if self._fileinfo["dtype"].fields is None or self._fields is None:
            ...
            in_data = {
                fld: np.empty((nrows,), dtype=self._fileinfo["dtype"].fields[fld][0])
        ...
        if mode == "random":
            with open(self._fileinfo["filename"]) as f:
                for idx, recidx in enumerate(indices):
                    f.seek(
                        self._fileinfo["offset"]
                        + recidx * self._fileinfo["recordsize"],
                        os.SEEK_SET,
                    )
            ...
            nrec = (
                10000000 // self._fileinfo["dtype"].itemsize
            )  # maximum number of records to read at one time
            ...
            with open(self._fileinfo["filename"]) as f:
                ...
                for b, n in blocks(blocksize, nrec):
                    f.seek(
                        self._fileinfo["offset"]
                        + (b + blockstart) * self._fileinfo["recordsize"],
                        os.SEEK_SET,
                    )
