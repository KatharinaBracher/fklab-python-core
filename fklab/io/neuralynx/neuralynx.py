...
from fklab.io.common.binary import accessor
...
__all__ = [
    ...
    "NlxEventTimes",
    "nlx_parse_log",
    "nlx_parse_lost_records",
    "nlx_parse_cheetah_errors",
]
...
_NLXVALUEHANDLERS = [
    ...
    (_NlxFloatArrayHandler, ["ADBitVolts", "InputRange", "ThreshVal"]),
    (_NlxIntArrayHandler, ["ADChannel", "Resolution", "DisabledSubChannels"]),
    (
        _NlxStringHandler,
        [
            "AcqEntName",
    ...
    (_NlxFeatureHandler, ["Feature"]),
    (_NlxEnableHandler, ["DspDelayCompensation"]),
    (
        _NlxColorTrackingHandler,
        ["IntensityThreshold", "RedThreshold", "GreenThreshold" "BlueThreshold"],
    ),
]
...
def nlx_parse_lost_records(root):
    ...
    h = NlxHeader(path)
    result = dict(
        DataSectionsLost=int(h.values["DataSectionsLost"]),
...
def nlx_parse_cheetah_errors(root, time_window=None):
    ...
    try:
        ...
        if len(t) == 0:
            ...
            b = np.logical_or(
                time_window.contains(t[:, 0])[0], time_window.contains(t[:, 1])[0]
            )
...
def readheader(file):
    ...
    if (
        len(hdr) != NLXHEADERSIZE
    ):  # or hdr[0:35]!="######## Neuralynx Data File Header":
...
def parseheader(hdr):
    ...
    for line in hdr:
        ...
        if param == "Feature":
            param = "Features"
            if param in list(parameters.keys()):
...
class NlxFileTimedBuffers(NlxFileBase):
    ...
    def __init__(self, source, dtype, **kwargs):
        ...
        if self.aliasing():
            warnings.warn(
                "High cut filter frequency is higher than half the sampling frequency. Data in this file is possibly affected by aliasing.",
                RuntimeWarning,
            )
    ...
    def _expand_timestamp(self, timestamp, scale=None):
        ...
        if scale is None:
            scale = np.uint64(
                self._convert_time_to_timestamp(1) / self.header["SamplingFrequency"]
            )
    ...
    def _convert_timestamp(self, timestamp):
        return self._expand_timestamp(
            self._convert_timestamp_to_time(timestamp),
            scale=1.0 / self.header["SamplingFrequency"],
        )
    ...
    def _convert_data(self, data):
        ...
        if self._scale_data:
            scale = (
                scale
                * self._header.getvalue("ADBitVolts").reshape(1, 1, self._nchan)
...
class NlxFileSpike(NlxFileTimedBuffers):
    ...
    def __init__(self, source, *args, **kwargs):
        ...
        waveform_accessor = accessor(
            dtype, "waveform", record_processor=self._convert_data
        )
        ...
        accessors = dict(
            ...
            sample_timestamp=(
                accessor(dtype, "timestamp", record_processor=self._expand_timestamp),
                None,
            ),
...
class NlxFileTahiti(NlxFileTimedBuffers):
    ...
    def __init__(self, source="", *args, **kwargs):
        ...
        data_by_sample_accessor = accessor(
            dtype, "data", sample_dim=0, record_processor=self._convert_data
        )
    ...
    def _search_time_indices(self, start=0.0, stop=1.0, extra=0):
        ...
        stop = min(
            bisect.bisect_right(self.data.original_timestamp, stop) + extra,
            self.nrecords,
        )
...
class NlxFileCSC(NlxFileTimedBuffers):
    ...
    def __init__(self, source="", *args, **kwargs):
        ...
        # call base class __init__
        NlxFileTimedBuffers.__init__(
            self, source, dtype, nchannels=nchan, nsamples=nsamples, **kwargs
        )
        ...
        accessors = dict(
            ...
            timestamp_by_sample=(
                accessor(
                    ...
                    record_shape=(nsamples,),
                    sample_dim=0,
                    record_processor=self._expand_timestamp,
                ),
                indexer,
            ),
            signal_by_sample=(signal_by_sample_accessor, indexer),
            signal=(
                accessor(dtype, "signal", record_processor=self._convert_data),
                None,
            ),
    ...
    def _convert_timestamp(self, timestamp):
        ...
        return self._expand_timestamp(
            timestamp=NlxTimestamp2Seconds(timestamp),
            scale=1.0 / self.header["SamplingFrequency"],
        )
...
def NlxEventTimes(
    path, eventstring="TTL Input on AcqSystem1_0 board 0 port 1 value (0x0080)."
):
...
class NlxVideoPoints:
    ...
    def __init__(self, data):
        ...
        self.rgb = np.concatenate(
            (
                self.red[:, :, np.newaxis],
                self.green[:, :, np.newaxis],
                self.blue[:, :, np.newaxis],
            ),
            axis=2,
        )
        self.bits = np.uint8(
            np.bitwise_and(np.right_shift(data, self.RAWBLUE - 3), 120)
            + np.bitwise_and(np.right_shift(data, self.PUREBLUE), 7)
        )
...
class NlxFileVideo(NlxFileBase):
    ...
    def __init__(self, source="", *args, **kwargs):
        ...
        accessors = dict(
            points=(
                accessor(dtype, "points", record_processor=self._convert_points),
                None,
            ),
            targets=(
                accessor(
                    dtype,
                    "targets",
                    record_processor=lambda targets: self._convert_points(
                        points=targets
                    ),
                ),
                None,
        ...
        if self.nrecords > 1:
            fs = 1.0 / np.mean(np.diff(self.data.time[0 : min(self.nrecords, 1000)]))
            # more than 1% difference
            if (
                np.abs(fs - self.header["SamplingFrequency"])
                / self.header["SamplingFrequency"]
                > 0.01
            ):
