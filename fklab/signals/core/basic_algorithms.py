"""
=============================================================
Basic algorithms (:mod:`fklab.signals.core.basic_algorithms`)
=============================================================

.. currentmodule:: fklab.signals.core.basic_algorithms
...
def remove_artefacts(
    signal, artefacts, time=None, axis=-1, window=1, interp="linear", fill_value=np.nan
):
        ...
        for region, regionsize in zip(idx, n):
            ...
            if region[0] < regionsize or region[1] >= len(signal) - regionsize:
                ...
                pre_reflect = 2 * np.take(signal, region[0], axis=axis) - np.take(
                    signal, np.arange(region[0] - regionsize, region[0]), axis=axis
                )
                post_reflect = 2 * np.take(signal, region[1], axis=axis) - np.take(
                    signal,
                    np.arange(region[1] + 1, region[1] + regionsize + 1),
                    axis=axis,
                )
...
_localextrema_discrete = (
    _localextrema_discrete_numba if _USE_NUMBA else _localextrema_discrete_python
)
...
def generate_windows(
    ...
    if not epochs is None:
        ...
        epochs = seg.segment_intersection(
            seg.segment_remove_overlap(epochs), [0, n - 1]
        )

        # divide each epoch into windows
        startidx = [
            np.arange(start, end - window_size + 1, window_shift, dtype=np.int)
...
def generate_trigger_windows(n, triggers, window, fs=1.0, start_time=0.0, epochs=None):
    ...
    elif len(window) != 2 or np.diff(window) <= 0.0:
...
def event_triggered_average(
    ...
    if interpolation is None:
        _, b = extract_trigger_windows(
            data, triggers=events, window=(lags[0], lags[-1]), fs=fs, start_time=t[0]
        )
        b = b.T
        lags = (np.arange(b.shape[1]) - (b.shape[1] - 1) / 2.0) / fs + (
            lags[0] + lags[-1]
        ) / 2.0
    else:
