"""
=================================================================
Open Ephys (:mod:`fklab.plot.openephys.openephys`)
=================================================================

.. currentmodule:: fklab.plot.openephys.openephys
...
def plot_recording( filename, start_time=0., channels=None, axes=None, spacing=0, xlim=None, ylim=None, labels=None, **kwargs ):
    ...
    **kwargs : extra keyword arguments for `fklab.plot.plot_signals` function.
    ...
    t = fklab.plot.core.utilities.RangeVector( f['recordings/0/data'].shape[0], start_time, 1./fs )
    
    h = fklab.plot.plots.plot_signals( t, [ fklab.plot.utilities.ColumnView(f['recordings/0/data'],k, lambda x, scale=0.001*bitvolts[k]: x*scale ) for k in channels ], spacing=spacing, axes=axes, **kwargs )
    ...
    scalebar = fklab.plot.core.artists.StaticScaleBar( location='right', size=[0.,0.1] , label='{value:.2f} mV', linewidth=2, offset=5 )
