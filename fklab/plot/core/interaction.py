...
def _on_press(event):
    ...
    cid_release = ax.figure.canvas.mpl_connect(
        "button_release_event", functools.partial(_on_release, shape=shape)
    )
    cid_motion = ax.figure.canvas.mpl_connect(
        "motion_notify_event", functools.partial(_on_move, shape=shape)
    )
...
class iEllipse(Ellipse):
    ...
    **kwargs : extra arguments for matplotlib.patches.Ellipse
    ...
    def __init__(self, center, size, orientation, **kwargs):
        self._interaction_handles = Line2D(
            [], [], markersize=8, color="k", marker="s", linestyle=""
        )
    ...
    def _interaction_start(self, event, extra):
        ...
        if not extra or int(extra["ind"][0]) == 0:
            ...
            self._click_angle = np.arctan2(
                event.ydata - self.center[1], event.xdata - self.center[0]
            )
            ...
            xm = np.cos(-self._origin_angle) * (event.xdata - self._origin[0]) - np.sin(
                -self._origin_angle
            ) * (event.ydata - self._origin[1])
            ym = np.sin(-self._origin_angle) * (event.xdata - self._origin[0]) + np.cos(
                -self._origin_angle
            ) * (event.ydata - self._origin[1])
    ...
    def _iresize(self, event):
        ...
        self.center = (
            self._origin[0]
            + np.cos(self._origin_angle) * dcx
            - np.sin(self._origin_angle) * dcy,
            self._origin[1]
            + np.sin(self._origin_angle) * dcx
            + np.cos(self._origin_angle) * dcy,
        )
...
class iRectangle(Rectangle):
    ...
    **kwargs : extra arguments for matplotlib.patches.Rectangle
    ...
    def __init__(self, center, size, orientation=0.0, **kwargs):
        self._interaction_handles = Line2D(
            [], [], markersize=8, color="k", marker="s", linestyle=""
        )
        ...
        kwargs["xy"] = center - np.array(
            [
                0.5 * size[0] * np.cos(orientation)
                - 0.5 * size[-1] * np.sin(orientation),
                0.5 * size[-1] * np.cos(orientation)
                + 0.5 * size[0] * np.sin(orientation),
            ]
        )
    ...
    def _update_interaction_handles(self):
        ...
        self._interaction_handles.set_xdata(np.cos(a) * x - np.sin(a) * y + xc)
        self._interaction_handles.set_ydata(np.sin(a) * x + np.cos(a) * y + yc)
        self._interaction_handles.set_marker((4, 0, (a * 180.0 / np.pi) - 45))

    @property
    def orientation(self):
        return self._angle * np.pi / 180.0

    @orientation.setter
    def orientation(self, value):
        self._angle = value * 180.0 / np.pi
    ...
    def _irotate(self, event):
        ...
        self.center = (
            self._origin
        )  # because we want rotation around center and not lower-left corner
        self._update_interaction_handles()
...
class iPolyline(Line2D):
    ...
    **kwargs : extra arguments for matplotlib.lines.Line2D
    ...
    def _interaction_start(self, event, extra):
        ...
        if "ind" in extra:
            ...
            if event.button == 1 and event.key == "control":  # left + control: rotate
                self._center = [
                    np.mean(np.asarray(self.get_xdata())),
                    np.mean(np.asarray(self.get_ydata())),
                ]
                self._click_angle = np.arctan2(
                    event.ydata - self._center[1], event.xdata - self._center[0]
                )
    ...
    def _rotate_curve(self, event):
        ...
        a = (
            np.arctan2(event.ydata - self._center[1], event.xdata - self._center[0])
            - self._click_angle
        )
        self._click_angle += a
...
class iPolygon(Polygon):
    ...
    **kwargs : extra arguments for matplotlib.patches.Polygon
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._interaction_handles = Line2D(
            [], [], markersize=8, color=self._handle_color, marker="s", linestyle=""
        )
        self._interaction_handles2 = Line2D(
            [], [], color=self._handle_color, linestyle="-"
        )
    ...
    def _interaction_start(self, event, extra):
        ...
        if "ind" in extra:
            ...
            if event.button == 1 and event.key == "control":  # left + control: rotate
                ...
                self._click_angle = np.arctan2(
                    event.ydata - self._center[0, 1], event.xdata - self._center[0, 0]
                )
                context = (self._rotate_curve, None)
    ...
    def _rotate_curve(self, event):
        ...
        xy = (
            np.dot(
                xy - self._center,
                np.array([[np.cos(a), -np.sin(a)], [np.sin(a), np.cos(a)]]).T,
            )
            + self._center
        )
        self.set_xy(xy)
    ...
    if len(points) == 0 or (len(points) == 1 and pointer is None):
        ...
        if len(points) == 2:
            if pointer is None:
                ...
                yp = np.sin(-angle) * (pointer[0] - points[0, 0]) + np.cos(-angle) * (
                    pointer[1] - points[0, 1]
                )
        else:
            yp = np.sin(-angle) * (points[2, 0] - points[0, 0]) + np.cos(-angle) * (
...
def _method_center_size(points, pointer):
    ...
    if len(points) == 0:
        ...
        if pointer is None:
            ...
            width = height = 2.0 * np.sqrt(np.sum((pointer - points[0]) ** 2))
            angle = np.arctan2(pointer[1] - points[0, 1], pointer[0] - points[0, 0])
    else:
        center = points[0]
        width = height = 2.0 * np.sqrt(np.sum((points[1] - points[0]) ** 2))
...
def _method_center_axes(points, pointer):
    ...
    if len(points) == 0:
        ...
        if pointer is None:
            ...
            width = 2.0 * np.sqrt(np.sum((pointer - points[0]) ** 2))
            height = 0.0
            angle = np.arctan2(pointer[1] - points[0, 1], pointer[0] - points[0, 0])
    else:
        center = points[0]
        width = 2.0 * np.sqrt(np.sum((points[1] - points[0]) ** 2))
        angle = np.arctan2(points[1, 1] - points[0, 1], points[1, 0] - points[0, 0])
        ...
        if len(points) == 2:
            if pointer is None:
                ...
                height = np.sin(-angle) * (pointer[0] - center[0]) + np.cos(-angle) * (
                    pointer[1] - center[1]
                )
        else:
            height = np.sin(-angle) * (points[2, 0] - center[0]) + np.cos(-angle) * (
                points[2, 1] - center[1]
            )
...
def create_boxed_shape(ax, method="center-size", shape="rectangle"):
    ...
    if shape == "rectangle":
        ...
        hShape = Ellipse(
            [np.nan, np.nan], 0, 0, linestyle=None, facecolor="b", alpha=0.3
        )
    ...
    def update_animation(ax, data, pointer):
        ...
        if shape == "rectangle":
            hShape.set_xy(
                ...
                - np.array(
                    [
                        0.5 * sz[0] * np.cos(angle) - 0.5 * sz[-1] * np.sin(angle),
                        0.5 * sz[-1] * np.cos(angle) + 0.5 * sz[0] * np.sin(angle),
                    ]
                )
            )
...
class BlockingCreateShape(BlockingInput):
    ...
    def __call__(
        ...
        cid = self._axes.figure.canvas.mpl_connect(
            "motion_notify_event", self.mouse_event_move
        )
