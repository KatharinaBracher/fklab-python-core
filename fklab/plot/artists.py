...
class FastSpectrogram(AxesImage):
    ...
    def __init__(
        ...
        super(FastSpectrogram, self).__init__(
            None, interpolation="none", clip_on=True, cmap=colormap
        )
    ...
    def set_overlap(self, val):
        ...
        if val < 0 or val >= 1.0:
            raise ValueError(
                "Overlap should be equal or larger than 0 and smaller than 1."
            )
        self._overlap = val
    ...
    def _prepare_data(self):
        ...
        b = fklab.segments.segment_split(
            [xlow, xhigh], size=self._window_size, overlap=self._overlap
        )
        ...
        if len(b) == 0 or len(data) == 0:
            ...
            try:
                data, f, _, _ = fklab.signals.multitaper._mtspectrum_single(
                    data,
                    bandwidth=self._bandwidth,
                    fs=self._fs / decimate_factor,
                    fpass=self._freq_range,
                )
                ...
                self._extent = [
                    self._start_time + b[0] / self._fs,
                    self._start_time + b[-1] / self._fs + self._window_size,
                    self._freq_range[0],
                    self._freq_range[1],
                ]
                ...
                self._A[np.arange(10), np.arange(10)] = 1.0
                self._A[np.arange(10), np.arange(10)[::-1]] = 1.0
                self._extent = [
                    self._start_time + b[0] / self._fs,
                    self._start_time + b[-1] / self._fs + self._window_size,
                    self._freq_range[0],
...
class EventImage(AxesImage):
    """Create Matplotlib image artist that represents binned events.
    ...
    def __init__(
        ...
        super(EventImage, self).__init__(axes, interpolation="none", clip_on=True)

        self._extent = [
            x[0] - 0.05 * (x[-1] - x[0]),
            x[-1] + 0.05 * (x[-1] - x[0]),
            self._y - 0.5 * self._height,
            self._y + 0.5 * self._height,
        ]

    @matplotlib.artist.Artist.axes.setter
    def axes(self, ax):
        super(EventImage, EventImage).axes.__set__(self, ax)
        self._prepare_data()
    ...
    def draw(self, renderer, *args, **kwargs):
        self._prepare_data()
        super(EventImage, self).draw(renderer, *args, **kwargs)
...
class FastLine(Line2D):
    ...
    def _prepare_data(self):
        ...
        idxend = np.minimum(
            len(self._full_xdata), bisect.bisect_right(self._full_xdata, xhigh)
        )
        ...
        if n > w:
            if self._random:
                idx = np.sort(
                    np.random.random_integers(low=idxstart, high=idxend, size=w)
                )
            else:
                f = int(n / w)
...
class StaticScaleBar(matplotlib.artist.Artist):
    ...
    def _update_location(self):
        ...
        if self._scalebar_location in ("left", "right"):
            if self._scalebar_location == "left":
                self.scalebar.set_xdata([0.0, 0.0])
                self.scalebar_label.set_x(0.0)
                self.scalebar_label.set_ha("right")
            else:
                self.scalebar.set_xdata([1.0, 1.0])
                self.scalebar_label.set_x(1.0)
            ...
            if self._scalebar_location == "bottom":
                self.scalebar.set_ydata([0.0, 0.0])
                self.scalebar_label.set_y(0.0)
                self.scalebar_label.set_va("top")
            else:
                self.scalebar.set_ydata([1.0, 1.0])
                self.scalebar_label.set_y(1.0)
    ...
    def _update_transforms(self):
        ...
        self.scalebar.set_transform(
            matplotlib.transforms.offset_copy(
                self.axes.transAxes, x=_x, y=_y, fig=self.axes.figure, units="points"
            )
        )
        ...
        self.scalebar_label.set_transform(
            matplotlib.transforms.offset_copy(
                self.axes.transAxes, x=_x, y=_y, fig=self.axes.figure, units="points"
            )
        )
        ...
        if sizex and labelx:
            bars = VPacker(
                children=[
                    bars,
                    TextArea(
                        labelx, minimumdescent=False, textprops=dict(color=color_txt)
                    ),
                ],
                align="center",
                pad=0,
...
class AxesMessage(matplotlib.artist.Artist):
    ...
    def __init__(
        ...
        alpha=0.75,
        color="red",
        textprop={"fontsize": 20, "fontweight": "bold"},
        *args,
        **kwargs
    ):
        ...
        self._text = Text(
            visible=False, color=color, alpha=alpha, zorder=100, **textprop
        )
        self.set_location(location)
...
class PositionTimeStrip(matplotlib.artist.Artist):
    ...
    def __init__(self, t, xy, roi, n, *args, **kwargs):
        ...
        self._future = [
            Line2D([], [], marker=".", color="red", linestyle="none", clip_on=True)
            for x in range(self._n + 1)
        ]
        self._past = [
            Line2D([], [], marker=".", color="blue", linestyle="none", clip_on=True)
            for x in range(self._n + 1)
        ]

        # construct artists for background
        img, _, _ = np.histogram2d(
            xy[:, 0], xy[:, 1], bins=[100, 100], range=roi, normed=True
        )
        img[img > 0.0] = 0.2
    ...
    def _prepare_data(self):
        ...
        for k in range(self._n + 1):
            ...
            xy = interp(t[k] + np.linspace(0.0, 5.0, 25))
            xy[:, 0] = xy[:, 0] * dt / (self._roi[0][1] - self._roi[0][0])
            xy[:, 0] += t[k] - 0.5 * dt
            ...
            self._all[k].set_extent(
                [t[k] - 0.5 * dt, t[k] + 0.5 * dt, self._roi[1][0], self._roi[1][1]]
            )
